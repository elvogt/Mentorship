...
def CreateFile(lURI, iURI): #Create directories based on the path 
    ...
    html = open(local_filename)                 #retrieves the header which contains the Content Type
    html = str(html)
    headers = str(headers)
    ...
    for x in range(1, length - 1):              #creates a directory for every step of the path except the last one which is the file name
        ...
        try: os.makedirs(directory)
            ...
            pass
    my_dir = paths[length-2]                    #directory to put file into
    if paths[length-1] == '':
        file_name = paths[length-2]
    else:
        file_name = paths[length-1]             #file name
    fname = my_dir + '/' + file_name            #specifies where to save the file and what to name it
    file = open(fname, 'w')
    file.write(headers + html)                            #puts the html (content) and headers into the file
    file.close()

    
def CreateJSON():
    saveLocation = "EMILY's/Mentorship"
    global fileName
    fileName = "Test"
    ...
    if localRecoveryLocation != None:			# Used to determine if there is a localRecoveryLocation
        if localRecoveryLocation == "none":
            pass
        else:
            newDictionary = FileRetrieve()
...
def FileDump(anchorDictionary, resourceDictionary, completeNameAnchor, completeNameResource):            
    ...
    ReadJSON(saveLocationcompleteNameAnchor)     #######STOPPED HERE#######
...
InitialURIs('http://i2.cdn.turner.com/cnnnext/dam/assets/160804100836-obama-clinton-hug-dnc-medium-plus-169.jpg')
# Mentorship
